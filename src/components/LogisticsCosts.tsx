...
import { Eye, Edit2, Trash2, ArrowUpDown, Search, Plus, Download, Truck } from 'lucide-react';
...
export default function LogisticsCosts() {
  ...
  const [editingCost, setEditingCost] = useState<LogisticsCost | null>(null);
  const [costs, setCosts] = useState<LogisticsCost[]>(initialLogisticsCosts);
  ...
  const modalFields = [
    ...
    { name: 'unitPrice', label: 'Cena jednostkowa (PLN)', type: 'number', required: true },
    {
      ...
      label: 'Kategoria',
      type: 'select',
      options: ['Transport', 'Delegacje', 'Sprzęt', 'Magazynowanie', 'Ubezpieczenia'],
      required: true
    }
  ];
  ...
  const handleEditCost = (cost: LogisticsCost) => {
    setEditingCost(cost);
    setIsModalOpen(true);
  };
      ...
      <div className="bg-white p-6 rounded-lg shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-semibold text-[#1E3A8A]">Koszty logistyki</h2>
          </div>
...
const groupByCategory = (costs: LogisticsCost[]) => {
  return costs.reduce((acc, cost) => {
    ...
    return acc;
  }, {} as Record<string, LogisticsCost[]>);
};

const calculateCategoryStats = (costs: LogisticsCost[]) => {
