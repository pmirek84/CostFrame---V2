...
import { Eye, Edit2, Trash2, ArrowUpDown, Search, Plus, Download } from 'lucide-react';
import type { InternalCost } from '../types';
import CostModal from './CostModal';

const initialInternalCosts: InternalCost[] = [
  {
    id: '1',
    name: 'Koszt pracy własnej',
    unit: 'godzina',
    unitPrice: 50,
...
export default function InternalCosts() {
  ...
  const [editingCost, setEditingCost] = useState<InternalCost | null>(null);
  const [costs, setCosts] = useState<InternalCost[]>(initialInternalCosts);
  ...
  const modalFields = [
    ...
    { name: 'unitPrice', label: 'Cena jednostkowa (PLN)', type: 'number', required: true },
    {
      ...
      label: 'Kategoria',
      type: 'select',
      options: ['Koszty operacyjne', 'Koszty stałe', 'Koszty sprzętu'],
      required: true
    },
    {
      name: 'period',
      label: 'Okres',
      type: 'select',
      options: ['jednorazowy', 'miesięczny', 'kwartalny', 'roczny']
    }
  ];
  ...
  const handleEditCost = (cost: InternalCost) => {
    setEditingCost(cost);
    setIsModalOpen(true);
  };
    ...
    <div className="space-y-6">
      <div className="bg-white p-6 rounded-lg shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-[#1E3A8A]">Koszty własne</h2>
...
const groupByCategory = (costs: InternalCost[]) => {
  return costs.reduce((acc, cost) => {
    ...
    return acc;
  }, {} as Record<string, InternalCost[]>);
};

const calculateCategoryStats = (costs: InternalCost[]) => {
